...
switch Par_info.boundhandling
    ...
    case 'fold'     % folding
        ...
        % Double check in case elements are still out of bound -- this is
        % theoretically possible if values are very small or large
        
    otherwise
        
        disp(['ABC_PMC:bound_handling:do not know this boundary ' ...
            'handling option! - treat as unbounded parameter space'])
...
if strcmp(Par_info.boundhandling,'reflect') || strcmp(Par_info.boundhandling,'fold')
    % Lower bound
    [ii_low] = find(x < min_d); x(ii_low) = min_d(ii_low) + ...
        rand(size(ii_low)).* ( max_d(ii_low) - min_d(ii_low) );
    % Upper bound
    [ii_up]  = find(x > max_d); x(ii_up)  = min_d(ii_up)  + ...
        rand(size(ii_up)).*  ( max_d(ii_up)  - min_d(ii_up)  );
end